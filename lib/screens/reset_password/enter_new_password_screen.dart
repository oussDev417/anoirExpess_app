...
class EnterNewPasswordScreen extends StatelessWidget {
  String phone;
  EnterNewPasswordScreen({required this.phone});
    ...
    return Scaffold(
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            Text(AppTranslations.of(context)!.text('restaurerMotDePasse'),
            ...
            Form(
              ...
              child: Column(
                children: [
                  AnoirInputComponent(isPassword: true,label: 'definirNouveauMotDePasse',controller: passwordController,),
                  SizedBox(height: 16,),
            ...
            AnoirPrimaryButton(text: 'enregistrer', onTap: () async {
              if(passKey.currentState!.validate()){
                if(confirmPasswordController.text.trim().isNotEmpty && passwordController.text.trim() == confirmPasswordController.text.trim()){
                  Loader.showLoader(context);
                  ApiResult result = await userController.resetPassword(phone,passwordController.text.trim());
                  Loader.offLoader(context);
                  if(result.success){
                    FlutterToastr.show(AppTranslations.of(context)!.text('felicitationsMotDePasseReset'), context,backgroundColor: Colors.green,textStyle: TextStyle(color: Colors.white,fontSize: 12));
